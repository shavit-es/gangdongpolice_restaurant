...
if(isset($_GET['id'])){
	$dessql="SELECT * FROM announcement WHERE id={$filtered_id};";
	...
	if(isset($_GET['page'])){
		...
		while($tablerow = mysqli_fetch_array($pageresult2)){
			...
			$table .= "<td class='text-center'><a href='announcement.php?id={$tablerow['id']}' class='link-secondary rows'>{$escaped_title}</a></td><td class='text-center' width='100px'>{$escaped_created}</td></tr><tr>";
		}
			$table = $table.'</tr>
					</tbody>
				</table>';
